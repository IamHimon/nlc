...
class GRUCellAttn(rnn_cell.GRUCell):
    def __init__(self, num_units, encoder_output, scope=None):
        ...
        :param num_units: cell个数 = self.size
        ...
        with vs.variable_scope(scope or type(self).__name__):
            with vs.variable_scope("Attn1"):
                ...
                phi_hs2d = tanh(rnn_cell_impl._linear(hs2d, num_units, True))
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with vs.variable_scope(scope or type(self).__name__):
            with vs.variable_scope("Attn2"):
                gamma_h = tanh(rnn_cell_impl._linear(gru_out, self._num_units, True))
            ...
            with vs.variable_scope("AttnConcat"):
                out = tf.nn.relu(rnn_cell_impl._linear([context, gru_out], self._num_units, True))
...
class NLCModel(object):
    def __init__(self, vocab_size, size, num_layers, max_gradient_norm, batch_size, learning_rate,
        ...
        self.size = size    # 第一层cell个数，也是embedding的维度
        ...
        self.source_tokens = tf.placeholder(tf.int32, shape=[None, None])   # 输入序列，
        self.target_tokens = tf.placeholder(tf.int32, shape=[None, None])   # 目标序列
    ...
    def setup_encoder(self):
        ...
        with vs.variable_scope("PryamidEncoder"):
            inp = self.encoder_inputs   # 输入序列的词向量矩阵
            mask = self.source_mask
            out = None
            for i in xrange(self.num_layers):
                with vs.variable_scope("EncoderCell%d" % i) as scope:
                    srclen = tf.reduce_sum(mask, reduction_indices=0)   # batch_size
                    out, _ = self.bidirectional_rnn(self.encoder_cell, inp, srclen, scope=scope)    # 构建双层rnn
                    dropin, mask = self.downscale(out, mask)
                    inp = self.dropout(dropin)  # 给下一层的输入引入dropout
    ...
    def setup_beam(self):
        ...
        def beam_step(time, beam_probs, beam_seqs, cand_probs, cand_seqs, *states):
            ...
            with vs.variable_scope("Logistic", reuse=True):
                ...
                logits2d = rnn_cell_impl._linear(do2d, self.vocab_size, True)
    ...
    def downscale(self, inp, mask):
        ...
        with vs.variable_scope("Downscale"):
            ...
            inp2d = tf.reshape(tf.transpose(inp, perm=[1, 0, 2]), [-1, 2 * self.size])  # 变成2*size
    ...
    def train(self, session, source_tokens, source_mask, target_tokens, target_mask):
        """
        训练入口，传入一batch数据，取回norm，loss
        :param session:
        :param source_tokens:   shape(max_length+pad_length, batch_size)
        :param source_mask:     shape(max_length+pad_length, batch_size)
        :param target_tokens:   shape(max_length+pad_length, batch_size)
        :param target_mask:     shape(max_length+pad_length, batch_size)
        :return:
        """
        input_feed = {}
    ...
    def decode_beam(self, session, encoder_output, beam_size=10):
        input_feed = {}
        input_feed[self.encoder_output] = encoder_output
        input_feed[self.keep_prob] = 1.
