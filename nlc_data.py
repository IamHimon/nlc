        ...
        for path in data_paths:
            with gfile.GFile(path, mode="rb") as f:
                ...
                for line in f:
                    ...
                    line = line.decode('utf-8')  # added by jujianxun
...
def sentence_to_token_ids(sentence, vocabulary,
                          tokenizer=None, normalize_digits=False):
    # sentence = sentence.decode('utf-8')  # added by jujianxun
    if tokenizer:
    ...
    return [vocabulary.get(re.sub(_DIGIT_RE, b"0", w.encode('utf-8')), UNK_ID) for w in words]
...
def prepare_nlc_data(data_dir, max_vocabulary_size, tokenizer=char_tokenizer, other_dev_path=None):
    """
    将训练集和验证集的token变成id表示，同时构建词典
    :param data_dir:
    :param max_vocabulary_size:
    :param tokenizer:
    :param other_dev_path:
    :return: 路径
    """
