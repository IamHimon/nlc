...
def train():
    ...
    x_train_path, y_train_path, x_dev_path, y_dev_path, vocab_path = nlc_data.prepare_nlc_data(
    ...
    with tf.Session() as sess:
        ...
        logging.info('Initial validation cost: %f' % validate(model, sess, x_dev_path, y_dev_path))
        ...
        while (FLAGS.epochs == 0 or epoch < FLAGS.epochs):  # 一共FLAGS.epochs轮训练
            epoch += 1
            current_step = 0

            ## Train
            epoch_tic = time.time()
            for source_tokens, source_mask, target_tokens, target_mask in pair_iter(x_train_path, y_train_path,
                                                                                    FLAGS.batch_size,
                                                                                    FLAGS.num_layers):
                # Get a batch and make a step.
                tic = time.time()
                print("data for train:")
                print('source_tokens', source_tokens)  # (?,batch_size)
                print('source_mask', source_mask)    # (?,batch_size)
                print('target_tokens', target_tokens)  # (?,batch_size)
                print('target_mask', target_mask)    # (?,batch_size)
                ...
                std_length = np.std(lengths)    # 计算标准差
            ...
            ## Validate
            valid_cost = validate(model, sess, x_dev_path, y_dev_path)
